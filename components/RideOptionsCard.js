...
const RideOptionsCard = () => {
  ...
  const arrayDataRides = Array();
  const rideData = query(collection(db, "TRIPS"));
  const pushRidesToArray = async () => {
    try {

      const querySnapshot = await getDocs(rideData)
      querySnapshot.forEach((document) => {
        arrayDataRides.push(document.data());
      });
      console.log(arrayDataRides)
    } catch (error) {
  ...
  const data = [
    {
      ...
      origin_description: 'Washington DC',
      destination_description: 'New York City, NY',
      departure_time: '9:00',
      arrival_time: '13:00',
      reviews: '4.9',
      price_per_seat: '11'
    ...
    },
    {
      ...
      origin_description: 'Washington DC',
      destination_description: 'Brooklyn, NY',
  ...
  return (
    <SafeAreaView style={tw``}>
      ...
      <FlatList
        ...
        renderItem={({ item: { id, name, origin_description, destination_description, image, departure_time, arrival_time, reviews, price_per_seat } }) => (
          <TouchableOpacity style={tw`justify-between px-3 py-3 bg-white m-2 rounded-xl shadow-lg`}
            onPress={() => selectRide(id)}
          >
            <View style={tw`flex-row justify-between `}>
              <Text style={tw`font-bold`}>{origin_description}</Text>
              <Icon
